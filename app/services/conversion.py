...
def convert_ppt_to_png(local_file_path: str, file_name: str, format: Literal["png", "pdf"]) -> str:
    try:
...
def handle_file_conversion(local_file_path: str, file_name: str, format: Literal["png", "pdf"]) -> str:
    try:
        logger.info(f"Local ppt file path: {local_file_path} - {file_name}")
        output_dir = os.path.join(PROJECT_ROOT, "tempfiles", file_name, "slides")
        logger.info(f"Output directory for PNGs: {output_dir}")
