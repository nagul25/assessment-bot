...
├── standards_setup.py                # One-time Azure Assistant setup script
├── standard_files.json               # Generated file IDs for standards (auto-generated)
├── technology_standard_list.csv       # Sample CSV data
├── requirements.txt                   # Python dependencies
├── env_example.txt                   # Environment variables template
├── files/                            # Static standards documents folder
│   └── (place your PDF standards here)
├── app/
│   └── services/
│       └── azure_assistant.py        # Azure Assistants API integration
└── README.md                         # This file
```
...
# Azure Assistant ID (generated by standards_setup.py)
AZURE_ASSISTANT_ID=asst_xxxxxxxxxxxx
```
...
## Azure Assistants API Integration

The system includes Azure Assistants API integration for architecture assessment using:
- **Static Standards (file_search)**: Pre-uploaded standard documents (EA principles, Cloud Principles, etc.)
- **Vision Capability**: Analyze user-uploaded PPT/PDF architecture diagrams as images
- **GPT-5 Model**: Advanced language model for assessment and recommendations

### Setting Up Standards

1. **Add Standard Documents**

   Place your standard PDF documents in the `files/` folder:
   ```
   files/
   ├── EA_Principles.pdf
   ├── Cloud_Principles.pdf
   └── Experian_Guidelines.pdf
   ```
...
2. **Run the Setup Script**
   ...
   The setup script uploads standards to Azure and creates the assistant:
   ```bash
   python standards_setup.py
   ```

   This will:
   - Upload all files from `files/` folder to Azure
   - Create a permanent assistant with `file_search` capability
   - Save file IDs to `standard_files.json`
   - Display the assistant ID to add to your `.env` file

3. **Configure Assistant ID**

   After running the setup script, add the generated assistant ID to your `.env`:
   ```bash
   AZURE_ASSISTANT_ID=asst_xxxxxxxxxxxx
   ```
...
```bash
# Initial setup - upload standards and create assistant
python standards_setup.py setup

# Update - add new files to existing assistant
python standards_setup.py update

# List - show current configuration
python standards_setup.py list
```

### Customizing the System Prompt

The assessment bot's behavior is controlled by the system prompt in `app/services/azure_assistant.py`. Edit the `ASSESSMENT_SYSTEM_PROMPT` variable to customize:

```python
ASSESSMENT_SYSTEM_PROMPT = """Your custom instructions here..."""
```

Then re-run the setup script to create a new assistant with the updated prompt.
...
**POST** `/api/poc/analyse`

Analyze architecture documents against standards:

```bash
curl -X POST "http://localhost:8000/api/poc/analyse" \
  -F "prompt=Evaluate this architecture against EA principles" \
  -F "files=@architecture.pptx"
```

**Parameters:**
- `prompt` (required): Assessment prompt/question
- `files` (optional): PPT/PDF files to analyze
...
```json
{
  ...
  "status": 200,
  "message": "Assessment handled successfully"
}
```
...
```bash
curl http://localhost:8000/api/poc/health
```

### How Assessment Works

1. **File Upload**: User uploads PPT/PDF via `/api/poc/analyse`
2. **Blob Storage**: Files are uploaded to Azure Blob Storage (backup)
3. **Conversion**: PPT/PDF converted to PNG images (one per slide/page)
4. **Azure Files**: PNGs uploaded to Azure Files API for assistant
5. **Thread Creation**: New thread created with prompt + image attachments
6. **Assistant Run**: Assistant analyzes images using Vision + searches standards
7. **Response**: Structured assessment returned to user

### Assessment Flow Diagram

```
User PPT/PDF → Blob Storage (backup)
            → Convert to PNGs
            → Upload to Azure Files API
            → Create Thread (prompt + images)
            → Run Assistant
                ├── file_search (standards)
